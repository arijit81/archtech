...
$result = $data->get_filtered_matches_csv($filters);
if (!$result) {
...
header('Content-Disposition: attachment; filename=Full-match-data-of-' . ucwords($umpire_name) . '-'.$time.'.csv');
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (isset($row['matches_record_venue'])) {
        $row['matches_record_venue'] = ucwords(strtolower($row['matches_record_venue']));
    }
